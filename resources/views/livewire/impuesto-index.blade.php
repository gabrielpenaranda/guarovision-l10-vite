<div class="container mx-auto py-4 max-w-4xl">
    <div class="flex pb-2 items-center justify-between">
        <h4 class="titulo">Impuestos</h4>
        @can('impuesto.create')
            <a href="{{ route('impuesto.create') }}" class="btn btn-new btn-xs sm:btn-sm mr-4">Nuevo</a>
        @endcan
    </div>
    <x-table>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col"
                            ...
                            wire:click="order('impuesto')">
                            impuesto<sup><i class="fa-solid fa-magnifying-glass"></i></sup>
                        ...
                        <th scope="col"
                            class="hidden sm:table-cell w-44 sm:w-72 px-6 py-3 text-left text-xs font-medium uppercase text-gray-500 tracking-normal cursor-pointer">
                            impuesto
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @foreach ($impuestos as $impuesto)
                        <tr>
                            <td class="px-6 py-4">
                                <span class="px-2 text-xs sm:text-sm leading-5 font-semibold hl-table">
                                    {{ $impuesto->impuesto }}
                                </span>
                            </td>

                            <td class="px-6 py-4">
                                <span class="hidden sm:table-cell px-2 text-xs sm:text-sm leading-5 font-semibold">
                                    {{ $impuesto->tasa }}
                                </span>
                            </td>
                    ...
                    <td class="px-6 py-4 text-sm font-medium text-center">
                        @if (Auth::check())
                            <form action="{{ route('impuesto.show-destroy', ['impuesto' => $impuesto->id]) }}"
                                method='GET'>
