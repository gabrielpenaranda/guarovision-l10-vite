...
@section('content')
    <div class="container mx-auto py-4 max-w-4xl">
        ...
        <form action="{{ route('recibo.store-credito', ['cliente' => $cliente->id]) }}" method="POST">
            @csrf
            <div class="bg-gray-100 pb-4 rounded-xl border-4">
                ...
                <div class="grid grid-cols-3 gap-4 mt-4 mx-8 sm:mx-14">
                    ...
                    <div class="col-span-2">
                        <label for="recibo_id" class="form-label">Recibo</label>
                        <select name="recibo_id" id="recibo_id" class="form-control w-full">
                            @foreach ($recibos as $recibo)
                                @php
                                    $saldo = number_format($recibo->saldo, 2, ',', '.');
                                @endphp
                                <option value="{{ $recibo->id }}">{{ $recibo->numero }} - {{ $recibo->concepto }}
                                    {{ $recibo->divisa->simbolo }} {{ $saldo }}
                                </option>
                            @endforeach
                        </select>
                    </div>
